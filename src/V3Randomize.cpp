    ...
    void visit(AstConstraintUnique* nodep) override {
        for (AstNode* itemp = nodep->rangesp(); itemp; itemp = itemp->nextp()) {
            if (AstVarRef* varrefp = VN_CAST(itemp, VarRef)) {
                ...
                varrefp->user1(true);
                if (AstNodeModule* modp = varrefp->classOrPackagep()) { modp->user1(true); }
                ...
                if (VN_IS(varp->dtypep()->skipRefp(), UnpackArrayDType)) {
                    ...
                    if (initTaskp) {
                        ...
                        // 1. Manually register the array with the SMT solver (replaces the dummy
                        // SV constraint) Syntax: write_var(data_ref, bit_width, "smt_name",
                        // is_array_flag)
                    ...
                    nodep->v3warn(E_UNSUPPORTED,
                                  "Constraint unique currently only supported for static arrays: "
                                      << varp->name());
                }
            }
        }
